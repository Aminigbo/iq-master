...
if (empty($input)) {
    ...
    $query = "INSERT INTO xtra_categories (date, category, no_of_qst, status, category_code,points,type,host)VALUES('$date','$input', '', 'inactive', '$id','$points', '', '$session_id')";
...
if (isset($_POST['all_category'])) {
    ...
    $sql = "SELECT * FROM xtra_categories WHERE type!='league' AND host='$user' ORDER BY id DESC "; 
$result = mysqli_query($conn, $sql); ?>
...
$id=generateKey($conn);
    ...
    echo $question ."<br>".$option1."<br>".$option2."<br>".$option3."<br>".$option4."<br>".$category;
...
if (isset($_POST['auto_insert_answered_questions'])) {
	...
	$query = "INSERT INTO answers (date, category, question, user, choice,answer,remark,claim,count)VALUES('$date','$qst_category', '$qst', '$user', '$choice', '$ans', '$remark','0','$count')";
mysqli_query($conn, $query);
...
if (isset($_POST['send-comment'])) {
    ...
    if (empty($comment)) {
        ...
        $query = "INSERT INTO coments (date, user, category,coment)VALUES('$date', '$user', '0','$comment')";
...
if (isset($_POST['clear_cach'])) {
    $category  = mysqli_real_escape_string($conn, $_POST['clear_cach']) ;
    $user  = mysqli_real_escape_string($conn, $_POST['user']) ;

    $sql1 = "DELETE  FROM answers WHERE category='$category' and user ='$user' ";
    mysqli_query($conn, $sql1);

    $query = "UPDATE qst_counter SET rating='0' WHERE user='$user' AND category='$category' ";
    mysqli_query($conn, $query);

    $query = "UPDATE qst_counter SET counting='1' WHERE user='$user' AND category='$category' ";
    mysqli_query($conn, $query);
}
